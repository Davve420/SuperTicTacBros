...
public class TicTacToeGame
{
    ...
    public TicTacToeGame(WebApplication app)
    {
        ...
        app.MapGet("/api/random-question", async () =>
        {
            try
            {
                ...
                if (question == null)
                {
                    return Results.NotFound("Inga frågor hittades");
                }
    ...
    {
        await using var cmd = db.CreateCommand("SELECT * FROM questions ORDER BY RANDOM() LIMIT 1");
        await using var reader = await cmd.ExecuteReaderAsync();
